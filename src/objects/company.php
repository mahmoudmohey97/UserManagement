...
class Company
{
    ...
    function readById($id)
    {
        $query = "select company.NAME , company.EMAIL, company.NUMEMPLOYEES , GROUP_CONCAT(INTERESTS) as interest_name 
                    ...
                    ( SELECT interest.ID FROM interest WHERE interest.COM_USERID = '{$id}' ) AND company.USERID = '{$id}' GROUP BY company.NAME";
        $querylocations = "select company.NAME , company.EMAIL, company.NUMEMPLOYEES , GROUP_CONCAT(LOCACATIONS) as locations 
                    FROM companylocation , company WHERE company.USERID= '{$id}' AND company.USERID = companylocation.USERID ";
    ...
    function read()
    {
        ...
        $querylocations = "select company.NAME , company.EMAIL, company.NUMEMPLOYEES , GROUP_CONCAT(LOCACATIONS) as locations 
                    FROM companylocation , company WHERE company.USERID = companylocation.USERID ";
        $stmt = $this->conn->query($query);
        return $stmt;
    }
